// ─────────────────────────────────────────────────────────────────────────────
// navigationService.js  —  Firebase Firestore version
//
// DROP-IN replacement for the old axios-based service.
// No backend server needed. All data lives in Firestore.
//
// Setup steps:
//   1. npm install firebase
//   2. Create frontend/src/config/firebase.js  (see instructions below)
//   3. Run the seed function once to populate Firestore (see seedFirestore below)
// ─────────────────────────────────────────────────────────────────────────────
...
import { db } from '../config/firebase'; // ← create this file (instructions below)

const LOCATIONS_COL = 'locations';

// ─── Helper: fetch all locations once ────────────────────────────────────────
async function fetchAllLocations() {
  const snapshot = await getDocs(collection(db, LOCATIONS_COL));
  return snapshot.docs.map((d) => ({ id: d.id, ...d.data() }));
}

// ─── Search locations by name / description / tags ───────────────────────────
//
// Returns: { success, count, data: Location[] }
//
export const searchLocations = async (searchTerm) => {
  try {
    ...
    const filtered = all.filter((loc) => {
      const nameMatch = loc.name?.toLowerCase().includes(lower);
      const descMatch = loc.description?.toLowerCase().includes(lower);
      const tagMatch = loc.tags?.some((t) => t.toLowerCase().includes(lower));
...
// ─── Get all locations (optional filters) ────────────────────────────────────
//
// filters: { building?, floor?, type? }
// Returns: { success, count, data: Location[] }
//
