...
export default function QRScannerScreen({ navigation }) {
  ...
  const handleBarCodeScanned = async ({ data }) => {
    ...
    try {
      ...
      // FIX: getAllLocations already returns the array, no need for response.data
    ...
    return (
      <SafeAreaView style={styles.centered}>
        ...
        <Text style={styles.permText}>
          Campus Navigator needs camera access to scan QR codes placed around the campus.
        </Text>
      ...
      <View style={styles.overlay}>
        ...
        <View style={styles.overlayBottom}>
          ...
          <View style={styles.controls}>
            ...
            {scanned && (
              <TouchableOpacity style={styles.controlBtn} onPress={() => setScanned(false)}>
...
const styles = StyleSheet.create({
  ...
  scanBox: { width: SCAN_SIZE, height: SCAN_SIZE, position: 'relative', overflow: 'hidden' },
  corner: { position: 'absolute', width: 24, height: 24, borderColor: '#00FF88', borderWidth: 3 },
  cornerTL: { top: 0, left: 0, borderRightWidth: 0, borderBottomWidth: 0 },
  cornerTR: { top: 0, right: 0, borderLeftWidth: 0, borderBottomWidth: 0 },
  cornerBL: { bottom: 0, left: 0, borderRightWidth: 0, borderTopWidth: 0 },
  cornerBR: { bottom: 0, right: 0, borderLeftWidth: 0, borderTopWidth: 0 },
  scanLine: { position: 'absolute', left: 0, right: 0, height: 2, backgroundColor: '#00FF88', shadowColor: '#00FF88', shadowOffset: { width: 0, height: 0 }, shadowOpacity: 0.8, shadowRadius: 4 },
  scannedOverlay: { ...StyleSheet.absoluteFillObject, backgroundColor: 'rgba(0,255,136,0.2)', justifyContent: 'center', alignItems: 'center' },
  scannedIcon: { fontSize: 64, color: '#00FF88' },
  scanHint: { color: '#fff', fontSize: 15, textAlign: 'center', marginBottom: 24, paddingHorizontal: 32 },
  controls: { flexDirection: 'row', gap: 32, marginBottom: 24 },
  controlBtn: { alignItems: 'center' },
  controlIcon: { fontSize: 28 },
  controlLabel: { color: '#fff', fontSize: 12, marginTop: 4 },
  backBtn: { paddingHorizontal: 32, paddingVertical: 12, borderRadius: 24, borderWidth: 1, borderColor: 'rgba(255,255,255,0.4)' },
  ...
  permBtn: { backgroundColor: '#00FF88', paddingHorizontal: 32, paddingVertical: 14, borderRadius: 14, marginBottom: 12 },
